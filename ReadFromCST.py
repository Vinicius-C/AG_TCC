...
class ReadFromCST:
    ...
    def s_cascade(x, y):
        ...
        s22 = y[3] + (y[2] * y[1] * y[3]) / denominador
    ...
    def plotar_S(self):  
        ...
        s21D = (file_s12d[:,1]+1j*file_s12d[:,2])
        s12D = (file_s21d[:,1]+1j*file_s21d[:,2])
        s22D = (file_s22d[:,1]+1j*file_s22d[:,2])

        s11P = (file_s11p[:,1]+1j*file_s11p[:,2])
        s12P = (file_s12p[:,1]+1j*file_s12p[:,2])
        s22P = (file_s21p[:,1]+1j*file_s21p[:,2])
        s21P = (file_s22p[:,1]+1j*file_s22p[:,2])
